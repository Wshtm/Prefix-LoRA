...
class LoRALayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # LoRA adjustment: apply low-rank transformation
        lora_adjustment = torch.einsum('oi,ir,sbr->sbo', self.lora_B, self.lora_A, x)
        # 返回叠加后的输出
        return base_output + self.scaling * lora_adjustment

def apply_lora_to_model(model, r, alpha):
    """
    遍历模型，将所有 nn.Linear 替换为 LoRALayer。
    Args:
        model: 待替换的模型。
        r: LoRA 的低秩分解的秩。
        alpha: LoRA 的缩放因子。
    ...
    for name, module in model.named_children():
        if isinstance(module, nn.Linear):
            ...
            # 递归替换子模块
            apply_lora_to_model(module, r, alpha)
"""
PrefixSummarizationModule 继承自 PrefixTransformer
该模块结合了 Prefix Tuning 和 LoRA 技术，通过超参数提供启用或禁用 Prefix Tuning 和 LoRA 的选项。
"""
class PrefixSummarizationModule(PrefixTransformer):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        # use_prefix_tuning 参数，控制是否使用Prefix Tuning
        self.use_prefix_tuning = hparams.use_prefix_tuning 

        if self.use_prefix_tuning=="yes":
            print("Prefix Tuning is enabled. Model will use prefix prompts.")
            self.model.config.use_prefix = True
        else:
            print("Prefix Tuning is disabled.")
            self.model.config.use_prefix = False
        ...
        # use_lora 参数，控制是否使用LoRA默认为 False
        self.use_lora = hparams.use_lora 
        if self.use_lora=="yes":
            print("Applying LoRA to the model...")
            apply_lora_to_model(self.seq2seq_model, r=16, alpha=32)
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--n_train", type=int, default=50000, required=False, help="# examples. -1 means use all.")
        parser.add_argument("--n_val", type=int, default=500, required=False, help="# examples. -1 means use all.")
        parser.add_argument("--n_test", type=int, default=10000, required=False, help="# examples. -1 means use all.")
        parser.add_argument(
...
def main(args, model=None) -> SummarizationModule:
    ...
    for name, param in model.named_parameters():
        ...
        if(param.requires_grad == True):
            train_num+=param.numel()
    # 统计总参数量和训练参数量
    print(f"total:{total_num},train:{train_num}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_prefix_tuning', type=str, default='no', help='')
    parser.add_argument('--use_lora', type=str, default='no', help='')
    args = parser.parse_args()
    print(f"useP:{args.use_prefix_tuning}")
    print(f"useL:{args.use_lora}")
